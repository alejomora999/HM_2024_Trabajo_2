...
% Grupo de investigación IDEAS.
    ...
    if particiones ~= 0
        ...
        ycdf = cumsum(fr);
        ypdf=fr/(x(2)-x(1));
        maximo1=max(ypdf)*0.1;
        for i = 1:length(ypdf)%for original
                if ypdf(i) < maximo1
                % Recorrer los siguientes n registros a partir de la ocurrencia
                    j=i;
                    a=5;
                    valor=[];
                    posicion=[];
                    while j<length(ypdf-i) && a>1 %%i=15,j=15
                        if ypdf(j) < maximo1
                            %disp(j);
                            valor = [valor,ypdf(j)];
                            posicion = [posicion,j];
                    ...
                    if numel(posicion) > 1
                        media = mean(valor);
                        for m=1:length(posicion-1)
                            ypdf(posicion(m))=media;
                        end    
                    end    

                end
        end
        ypdf= medfilt1(ypdf,90);
    else
...
function ypdf = recursiveMean(ypdf, i, maxDepth)
    % Verifica la condición base para detener la recursión
    if i > length(ypdf)
        return;
    else
        % Calcula la media de los maxDepth últimos elementos hasta el índice i
        if i > maxDepth
            % Selecciona los elementos desde i-maxDepth+1 hasta i para incluir maxDepth elementos
            ypdf(i) = mean(ypdf((i-maxDepth+1):i));
        end
        % Llama a la función recursivamente para el próximo índice
        ypdf = recursiveMean(ypdf, i + 1, maxDepth);
    end
end